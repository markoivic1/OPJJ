...
public class SmartScriptParser {
    ...
    private Node parse() {
        ...
        while (true) {
            ...
            if (tokenTypeIs(TokenType.TAG_OPENED)) {
                ...
                } else if (((String) lexer.getToken().getValue()).toLowerCase().equals("now")) {
                    NowNode nowNode = parseNow();
                    ((Node) node.peek()).addChildNode(nowNode);
                } else {
                    throw new SmartScriptParserException();
                }
    ...
    private NowNode parseNow() {
        ElementString string;
        lexer.nextToken();
        if (!tokenTypeIs(TokenType.TAG_CLOSED)) {
            string = new ElementString((String) lexer.getToken().getValue());
        } else {
            lexer.setState(LexerState.TEXT);
            return new NowNode(new ElementString("yyyy-MM-dd HH:mm:ss"));
        }
        lexer.setState(LexerState.TEXT);
        return new NowNode(string);
    }
    /**
